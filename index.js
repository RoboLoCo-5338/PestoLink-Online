...
async function renderLoop() {
    ...
    if (localStorage.getItem(toggleKeyboardWASD.id) === 'true') {
        ...
        if (Array.from(navigator.getGamepads()).filter(gamepad => gamepad).length < 1 || (Array.from(navigator.getGamepads()).filter(gamepad => gamepad).length == 1 && localStorage.getItem(toggleDualControllers.id) === 'true' && localStorage.getItem(toggleKeyboardWASD.id) === 'true')) buttonElements.forEach((button) => button.style.background = 'grey');
